...
with tf.variable_scope('conv1') as scope:
    ...
    c1 = tf.nn.conv2d(images, kernel1, strides=[1, 1, 1, 1], padding='SAME')
    

    # apply relu on the sum of convolution output and biases
    conv1 = tf.nn.relu(c1 + biases1, name=scope.name)    
...
with tf.variable_scope('conv2') as scope:
    ...
    biases2 = tf.get_variable('biases', [64],
                             initializer=tf.random_normal_initializer())
    c2 = tf.nn.conv2d(pool1, kernel2, strides=[1, 1, 1, 1], padding='SAME')
    conv2 = tf.nn.relu(c2 + biases2, name=scope.name)
